...
This folder is the entry point for the dynamic point cloud sequences used in the OpenDPC subjective experiment. The total payload is large, so the per-frame `.ply` data is **hosted on Google Drive** rather than committed to the Git history. This README documents what the dataset contains, how to download it, and how to point the [`Software/`](../Software) viewer at it.
